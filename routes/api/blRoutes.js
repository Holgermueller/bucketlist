...
bucketListRouter.route("/").get(function(req, res) {
  bucketListController.find(function(err, bucketList) {
    err ? console.log(err) : res.json(bucketList);
  });
});
...
bucketListRouter.route("/update/:id").post(function(req, res) {
  bucketListController.findById(req.params.id, function(err, listItem) {
    !listItem
      ...
      : ((listItem.itemOnList = req.body.itemOnList),
        (listItem.status = req.body.status),
        (listItem.dateCreated = req.body.dateCreated),
